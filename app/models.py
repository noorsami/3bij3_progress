...
class User(UserMixin, db.Model):
    ...
    points_stories = db.relationship(
        'Points_stories', backref='user', lazy='dynamic')
    points_invites = db.relationship(
        'Points_invites', backref='user', lazy='dynamic')
    points_ratings = db.relationship(
        'Points_ratings', backref='user', lazy='dynamic')
    points_logins = db.relationship(
        'Points_logins', backref='user', lazy='dynamic')
    ...
    recommended_num = db.relationship(
        'Num_recommended', backref='user', lazy='dynamic')
    divers = db.relationship('Diversity', backref='user', lazy='dynamic')
    again_showed = db.relationship(
        'Show_again', backref='user', lazy='dynamic')
...
Similarities = db.Table('similarities',
                        ...
                        db.Column('id_old', db.Integer,
                                  db.ForeignKey('news_sel.id')),
                        db.Column('id_new', db.String(500),
                                  db.ForeignKey('all_news.id')),
