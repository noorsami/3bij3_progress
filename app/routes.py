...
from app.models import User, News, News_sel, Category, Points_logins, Points_stories, Points_ratings, Num_recommended, Show_again, Diversity
...
def points_overview():
    if current_user.is_authenticated:
        ...
        points = points_stories + points_ratings + points_logins
        if group == 4:
    ...
    return dict(points = points, points_ratings = points_ratings, points_stories = points_stories, points_logins = points_logins, group = group, phase = phase_completed, rest = rest)
...
def get_points():
    ...
    points_list = [points_stories_all, points_ratings_all, points_logins_all]
    ...
    points_overall = [sum(item) for item in zip(points_stories_all, points_logins_all, points_ratings_all)]
